...
def SEIR_results(t,N,
    ...
    for i in range(0,t.shape[0]):
        if (t[i] > InterventionTime and t[i] <= InterventionTime2): # Mon 16th March - Mon 23rd March - week before lockdown
            ...
            beta_hat[i] = (InterventionAmt3)*R0*(1/((p_sym/gamma) + ((1-p_sym)/gamma_asym)))
            beta[i] = beta[i-1]
            c[i] = beta_hat[i] / beta[i]
        elif (t[i] > InterventionTime4 and t[i] < InterventionTime4 + duration): # 28th May onwards - lockdown eased
            beta_hat[i] = (InterventionAmt4)*R0*(1/((p_sym/gamma) + ((1-p_sym)/gamma_asym)))
            beta[i] = beta[i-1]
            c[i] = beta_hat[i] / beta[i]
        elif (t[i] > InterventionTime + duration):
            beta_hat[i] = 0.5*R0*(1/((p_sym/gamma) + ((1-p_sym)/gamma_asym)))
            beta[i] = beta[i-1]
            c[i] = beta_hat[i] / beta[i]
    ...
    ker_asym = np.exp(-gamma_asym*t)
    ker_int_asym = np.trapz(ker_asym, t)
    ...
    for i in range(0,t.shape[0]):
        ...
        R_t[i] = (bcs[i]/N) * (p_sym*ker_int + (1-p_sym)*ker_int_asym) # Including contribution from both symptomatic and asymptomatic infectious
