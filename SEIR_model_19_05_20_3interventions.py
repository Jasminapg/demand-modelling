...
import pandas as pd
import SEIR_model_functions_19_05_20 as SEIRfun
...
fit_params = pd.read_csv('model_fitted_parameters_1805.csv', delimiter = ',')
...
# 1st Intervention
InterventionAmt   = 0.7 # effect of intervention in reducing transmission = fractional reduction in R0
# 2nd Intervention 
InterventionAmt2   = 0.23 # effect of intervention in reducing transmission = fractional reduction in R0
# Hypothetical interventions for easing lockdown:
InterventionTime3_day = 43946 #45000 # 43961 = Sun 10th May - Plan for easing announced. 
InterventionTime4_day = 45000 #InterventionTime3_day+30 or 44000 
InterventionAmt3 = 3.2/3.2
InterventionAmt4 = InterventionAmt3
...
#        InterventionAmt4 = x['InterventionAmt4']
        ...
        resid4 = (model_interp_GA-GA)#/model_interp_GA # residual between model and actuals
        ...
        resid5 = (model_interp_ICU-ICU)#/model_interp_ICU # residual between model and actuals
        ...
        resid6 = (model_interp_discharge-discharged)#/model_interp_discharge # residual between model and actuals
        ...
        obj = np.concatenate((resid3,resid4,resid5,resid6)) # residual between model and actuals. concatenate to get flat column with all residuals in one
        return obj
            
    #%% create a set of Parameters and their inital values and bounds
    params = Parameters()
#    params.add('R0', value=2.6, min=2.0, max=4.0)
...
#    params.add('D_recovery_GA', value=8, min=2, max=12)
#    params.add('D_recovery_ICU', value=13, min=8, max=20)
#    params.add('D_death_GA', value=8, min=2, max=15)
#    params.add('D_death_ICU', value=13, min=8, max=20)
#    
#    params.add('P_SEVERE', value=0.044, min=0.01, max=0.05)
#    params.add('p_ICU', value=0.18, min=0.15, max=0.25)
#    params.add('CFR_GA', value=0.26, min=0.15, max=0.4)
#    params.add('CFR_ICU', value=0.5, min=0.35, max=0.65)
...
#    params.add('p_sym', value=0.7, min=0.3, max=1)
    ...
    params.add('InterventionAmt3', value=0.25, min=0.1, max=0.6)
...
if do_scenarios == 1: # 1 to do scenario mapping
    ...
    # set range of options for fractional change in R0 before intervention after Intervention 3 and 4
    dr = 0.05
    r_max = 1
    InterventionAmt3_range = np.linspace(0.0,r_max,int((r_max+dr)/dr))
    InterventionAmt4_range = np.linspace(0.0,r_max,int((r_max+dr)/dr))
    
    hosp_peak = np.empty([InterventionAmt3_range.shape[0],InterventionAmt4_range.shape[0]])
    hosp_peak_date = np.empty([InterventionAmt3_range.shape[0],InterventionAmt4_range.shape[0]])
    ...
    for x in range(0,InterventionAmt3_range.shape[0],1):
...
#                InterventionTime4 = InterventionTime4_day-t0
                ...
                # Define Intervention 3 and 4 parameters
                InterventionTime3 = InterventionTime3_day-t0
                InterventionTime4 = InterventionTime4_day-t0
                InterventionAmt3 = InterventionAmt3_range[x]
...
#            SEIR_table_list.append(SEIR_table_list[1] + SEIR_table_list[2] + SEIR_table_list[3]) # NEL sum of 3 trusts
            ...
            hosp_peak[x,y] = stats_list[-1][0] # NEL sum hospitalisation peak
            hosp_peak_date[x,y] = stats_list[-1][1] # NEL sum hospitalisation peak date
...
#            plt.figure()
            #plt.plot(t_final-t0, SEIR_table_list[n][:,idx_cases], color='gold',linestyle=pred_lines) # total cases
            plt.plot(t_final, SEIR_table_list[n][:,hosp_col_idx], '-b',linestyle=pred_lines) # hospitalised daily
            plt.plot(t_final, SEIR_table_list[n][:,idx_hospitalised_ICU], '-r',linestyle=pred_lines) # ICU daily
#            plt.plot(t_final-t0, SEIR_table_list[n][:,idx_R_hospitalised], '-g',linestyle=pred_lines) # recovered from severe
#            plt.plot(t_final-t0, SEIR_table_list[n][:,idx_R_fatal], '-k',linestyle=pred_lines) # deaths
#            if region_list[n] == 'NEL':
#                #plt.plot(data_cases.date-t0, data_cases.iloc[:,1], color='gold', marker='o')
