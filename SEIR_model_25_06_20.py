...
import pandas as pd
import SEIR_model_functions_25_06_20 as SEIRfun
...
fit_params = pd.read_csv('model_fitted_parameters_1806.csv', delimiter = ',')
...
#        t_train = 
    ...
    params.add('P_SEVERE', value=0.044, min=0.03, max=0.06)
...
if plot_results == 1:
    ...
    for n in range(0,fit_params.shape[1]-1,1): # for each region
        ...
        CFRStepTime = CFRStepTime_day-t0
        CFRStepTime2 = CFRStepTime2_day-t0
        CFR_scalar = fit_params.iloc[20,n+1]
        D_recovery_scalar = fit_params.iloc[21,n+1]
        DRecStepTime = DRecStepTime_day-t0
        DRecStepTime2 = DRecStepTime2_day-t0
...
if do_scenarios == 1: # 1 to do scenario mapping
    ...
    c_max = 17.768 # value of c when InterventionAmt4 = 1
    c_max_int = 8
    dc = 0.25
